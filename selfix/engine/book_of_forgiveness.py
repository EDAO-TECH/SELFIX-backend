...
def _load_manifest():
    ...
    # ✅ Only verify signature if it exists
...
def seal_file(path, strict=False):
    ...
    if strict and os.path.exists(TARGET_LIST):
        ...
        if path not in targets and name not in targets:
            print(f"⛔ Not in forgiveness_targets.txt: {path}")
            return
    ...
    try:
        ...
        print(f"❌ Failed: {e}")
        _log(f"QC FAILED: {name} — {e}")
        return

    shutil.copy2(path, f"{FILES_DIR}/{name}")
    file_hash = _hash(path)

    forgiven_by = input("📜 Forgiven by: ")
    reason = input("✏️ Reason for sealing: ")
    alias = input("🔖 Optional alias: ")
    ...
    _log(f"SEALED: {name} ✅")
    print(f"✅ {name} sealed and certified.")
...
def restore_file(name):
    ...
    print(f"♻️ Restored: {name}")

    # Post-restore verification
    if _hash(dst) == manifest[name]["hash"]:
        print("✅ Integrity verified after restore")
        _log(f"VERIFY OK: {name}")
    else:
        print("❌ Verify failed after restore")
